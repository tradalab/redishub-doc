...
export function DownloadClient({dict}: { dict: DownloadDict }) {
  ...
  const buttonLabel = dict.button.replace("{target}", `${MATRIX[os].label} (${arch})`)

  return (
    <main className="mx-auto max-w-3xl px-6 py-24">
      <h1 className="text-4xl font-bold text-neutral-900 dark:text-neutral-100">{dict.title}</h1>

      <p className="mt-4 text-neutral-600 dark:text-neutral-400">{dict.subtitle}</p>

      <div className="mt-10">
        <label className="block text-sm font-medium mb-3">{dict.osLabel}</label>
      ...
      <div className="mt-8">
        <label className="block text-sm font-medium mb-3">{dict.archLabel}</label>
